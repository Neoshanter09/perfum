    ...
    <x-app-layout>
        <x-slot name="header">
                ...
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    {{ __('phone Advertisement') }}
                </h2>
           
        </x-slot>

        <div class="container max-w-6xl mt-5"><!--p-3 -->
            <div class="flex-1  bg-white p-4"><!-- p-4-->
                        ...
                        <table class="min-w-full w-full table-auto bg-white border border-gray-200" id="table1">
                            ...
                            <tbody>
                                ...
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="py-3 px-4 border-b border-gray-200">{{$mobilephone->user->name}}</td>
                                    <td class="py-3 px-4 border-b border-gray-200">{{$mobilephone->topic}}</td>
                                    <td class="py-3 px-4 border-b border-gray-200 break-words max-h-24 overflow-auto">{{$mobilephone->description}}</td>
                                    <td class="py-3 px-4 border-b border-gray-200">{{$mobilephone->location}}</td>
                                    <td class="py-3 px-4 border-b border-gray-200">{{$mobilephone->phone}}</td>
                                    <td class="border-b border-gray-200">@if($mobilephone->image)
                                        <img src="{{ asset('storage/' . $mobilephone->image) }}" alt="item Image" width="100">
                                          @endif</td>
                                    <td class="py-3 px-4 border-b border-gray-200">{{$mobilephone->category}}</td>
                                    <td class="py-3 px-4 border-b border-gray-200">{{$mobilephone->price}}</td>

                                    
                                    
                                    <td class="py-3 px-4 border-b border-gray-200">
                                        <div class="flex justify-start space-x-2">
                                            <a href="{{route('phone_edit',$mobilephone->id)}}" class="btn btn-outline-warning">update</a>
                                        <form action="{{route('user_phone_delete',$mobilephone->id)}}" method="post">
                                            @csrf
                                            @method('delete')
